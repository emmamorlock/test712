...
ARG TEST712_VERSION=master

# add key
RUN  mkdir -p ~/.ssh && ssh-keyscan -t rsa github.com >> ~/.ssh/known_hosts

# Replace git URL below to point to your git repository 
RUN  git clone https://github.com/emmamorlock/test712.git \
    # replace my-edition with name of your app
    && cd test712 \
    && echo Checking out ${TEST712_VERSION} \
    && git checkout ${TEST712_VERSION} \
    && ant
...
# replace my-edition with name of your app
COPY --from=tei /tmp/test712/build/*.xar /exist/autodeploy/
